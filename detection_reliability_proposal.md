...
1. 기술적 입력 규격 (Input Specs)
   입력 차원: $(1, 20, 224, 224, 3)$ (Batch, Time, Height, Width, Channel)
   전처리:
   정규화: $Pixel / 255.0$ (0.0~1.0 실수화)
   색상 순서: BGR (iOS/Android의 RGB를 반드시 BGR로 변환)
   데이터 구성: 20개 프레임의 시계열 유지 (Index 0: 가장 과거, Index 19: 가장 최신)
2. 탐지 및 신뢰도 산출 알고리즘
   이동 평균 필터 ($N=5$): 1초 단위 추론 결과($S_t$) 5개를 평균 내어 노이즈 제거.
   임계값 재설정: \* SAFE ($0.0 \le C_t < 0.5$): 정상 영상.WARNING ($0.5 \le C_t < 0.7$): 주의 필요 (오탐지 방지를 위해 0.5~0.7 구간 설정).DANGER ($0.7 \le C_t \le 1.0$): 딥페이크 확률 매우 높음.
3. 필수 빌드 설정 (Critical)
   TFLite 연산자: SELECT_TF_OPS 활성화 필수 (Bi-LSTM 레이어 지원용).
   백본: MobileNetV3-Small (저사양 기기 최적화).
